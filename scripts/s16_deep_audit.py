#!/usr/bin/env python3
"""S16 DEEP AUDIT — the e2e audit across ALL 23 companies (the user ask:
"fully audit all the current process again e2e from data collection,
processing to enrichment ... upstream you are not missing anything,
downstream you are not leaving data on the table").
...
pass_fail(watch_labels == want, "D7 watch config roster == 23",
          f"{len(watch_labels)} watches")
pass_fail(h1b_labels == want, "D7 h1b employers roster == 23",
...
for label in ALL23:
    ...
    if len(idx) and matched < len(idx) * 0.15 and label not in (
            "faradayfuture", "moonshot", "alibaba"):
...
for f in fails:
    print(f)
with open("/tmp/s16_audit_results.txt", "w") as fh:
    fh.write("\n".join(results) + "\n")
print("(full lines → /tmp/s16_audit_results.txt)")
