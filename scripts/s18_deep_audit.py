#!/usr/bin/env python3
"""S18 DEEP AUDIT — the e2e audit across ALL 30 companies (the S18
go-broader census round). Extends S17's dimensions with the S18 census
boards (ashby hoyoverse + lever plus-2 + paylocity unitedimaging).
...
Run: python3 /home/z/my-project/job-sourcing-research/scripts/s18_deep_audit.py
"""
import csv
import json
import random
...
S18_NEW3 = ["hoyoverse", "plusai", "unitedimaging"]
ALL23 = S16_NEW10 + PRIOR13 + S17_NEW4 + S18_NEW3   # name kept (callers); it IS 30
...
for label in ALL23:
    ...
    if n < 30:
        ...
        # S18: an EMPTY li_index (0 cards on the whole surface) is
        # itself the verdict — 0% is the only honest ratio then
        # (unitedimaging: 41 rows, no LI cards at all — the first
        # n>=30 board in this class).
        idx_n = len(load_jsonl(
            DATA / f"{label}_us_fulltime.li_index.jsonl"))
        if idx_n == 0:
            pass_fail(ratio == 0, f"D3 {label} join (n={n})",
                      f"matched {matched}/{n} ({ratio:.0%}) — EMPTY "
                      "li_index (no LI surface; 0% is the verdict)")
        else:
...
pass_fail(watch_labels == want, "D7 watch config roster == 30",
          f"{len(watch_labels)} watches")
pass_fail(h1b_labels == want, "D7 h1b employers roster == 30",
...
for label, spec in [("moonshot", "ats:ashby:moonshot"),
                    ...
                    ("hoyoverse", "ats:ashby:hoyoverse"),
                    ("plusai", "ats:lever:plus-2"),
                    ("unitedimaging",
                     "ats:paylocity:d527ad39-680d-45fa-9178-38a81898aec2")]:
    dump_n = len(load_csv(label))
    tt = None if label in ("moonshot", "unitedimaging") else "Full time"
    try:
        site_boards._CACHE.clear()
        rows, meta = site_boards.list_board(
...
for label, want_tt in [("shein", "Full time"),
    ...
    if not rows:
        ...
        continue
    if want_tt is None:
        # xiaohongshu/unitedimaging: the API serves no employment type
        # — 100% blanks (the paylocity refusal class)
...
for label, spec, tt in [
        ...
        ("hoyoverse", "ats:ashby:hoyoverse", "Full time"),
        ("plusai", "ats:lever:plus-2", "Full time"),
        ("unitedimaging",
         "ats:paylocity:d527ad39-680d-45fa-9178-38a81898aec2", None)]:
    dump_n = len(load_csv(label))
    try:
        site_boards._CACHE.clear()
        rows, meta = site_boards.list_board(
